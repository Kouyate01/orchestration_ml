...
def build_model_specs() -> list[ModelSpec]:
    return [
        ModelSpec("random_forest", RandomForestClassifier(random_state=RANDOM_STATE), {"clf__n_estimators": [100], "clf__max_depth": [None, 10]}),
        ModelSpec("xgboost", XGBClassifier(random_state=RANDOM_STATE, eval_metric="logloss", n_jobs=-1), {"clf__n_estimators": [100], "clf__learning_rate": [0.1]}),
        ModelSpec("lightgbm", LGBMClassifier(random_state=RANDOM_STATE, verbose=-1), {"clf__n_estimators": [100], "clf__learning_rate": [0.1]}),
    ]
...
def optimize_model(spec, x_train, y_train, x_test, y_test, cv=5, scoring="roc_auc") -> FitResult:
    ...
    return FitResult(spec.name, best, search.best_params_, float(search.best_score_), float(f1_score(y_test, preds)), float(roc_auc_score(y_test, proba)), preds)

def describe_registered_version(name, version, result, cv, scoring):
    client = mlflow.MlflowClient()
    client.update_model_version(name=name, version=str(version), description=f"Modele {result.name}")
...
def log_run_to_mlflow(result, x_test, y_test, cv, scoring, register_as=None, is_best=False) -> None:
    with mlflow.start_run(run_name=result.name, nested=True):
        ...
        try:
            ...
            logger.warning(f"Impossible de sauvegarder le modele dans MLflow a cause d'une restriction de securite : {e}")
...
def train_all(cv=5, scoring="roc_auc", use_mlflow=True):
    ...
    if use_mlflow:
        with mlflow.start_run(run_name="compare-models"):
            ...
            for r in results:
                log_run_to_mlflow(r, x_test, y_test, cv, scoring, register_as=MODEL_NAME if r is best else None, is_best=(r is best))

    MODEL_DIR.mkdir(parents=True, exist_ok=True)
    joblib.dump(best.best_estimator, MODEL_DIR / "model.joblib")
    logger.info("Entrainement complet ! Meilleur modele sauvegarde localement et graphiques generes.")
