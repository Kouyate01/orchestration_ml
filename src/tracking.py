...
# On utilise ton dossier src.config
...
def setup_experiment() -> None:
    ...
    logger.info("Experience '%s' configuree (ID: %s)", MLFLOW_EXPERIMENT, experiment.experiment_id)

def log_dataset(df: pd.DataFrame, context: str, name: str = "dataset") -> None:
    """Logger un dataset MLflow avec tracabilite."""
    ...
    logger.info("Dataset '%s' loggue dans le contexte '%s'", name, context)